---
title: "Reconstrucción Ancestral de Secuencias - GCAATAGC - PCC_7716"
author: "Eduardo PM"
date: "2023-03-30"
...
#setwd("/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/NO_PALINDROMES/GCAATAGC")
knitr::opts_knit$set(root.dir = "/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/NO_PALINDROMES/GCAATAGC")
```
...
```{r}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/NO_PALINDROMES/GCAATAGC
#setwd("/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/NO_PALINDROMES/GCAATAGC")
Counts <- read.table("../../Markov_count_GCCATTGC_2023-5-8_13hrs20mins_Octanuc_.txt", sep = "\t", header = TRUE)
...
Aqui podemos ver que  **PCC_7716** es la que tiene mas sitios (**403**) seguido de **NIES-4071** y **NIES-4105** con **101**.


```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/NO_PALINDROMES/GCAATAGC
awk '{if ($5>=1 && $2=="Calothrix_sp_PCC_7716"){print $1}}' ../../Markov_count_GCCATTGC_2023-5-8_13hrs20mins_Octanuc_.txt >Ortologos_GCAATAGC_Calothrix_sp_PCC_7716.txt
```

**Ortologos_CGGCGCCG_Calothrix_sp_NIES-4071.txt** contiene 115 ortólogos los cuales estan presentes en las 6 especies y tienen por lo menos una ocurrencia del palindromo **CGGCGCCG**
```{bash, eval = FALSE}
###
5352_secA.fna
5354_sat.fna
5358_OmpA_family_protein.fna
5398_glycosyltransferase.fna
5444_thiamine_pyrophospha...fna
5445_trifunctional_serine...fna
5445_trifunctional_serine...fna
5474_hslO.fna
5498_glycosyltransferase_...fna
5518_ABC_transporter_tran...fna
###
```
...
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/NO_PALINDROMES/GCAATAGC
mkdir Orthologues_GCAATAGC_PCC_7716  ## Creo Carpeta
cd ../../gbff_homologues/CalothrixspNIES-3974NIES-3974_f0_0taxa_algOMCL_e0_ ## Entro a la carpeta de homólogos
for word in $(cat ../../NO_PALINDROMES/GCAATAGC/Ortologos_GCAATAGC_Calothrix_sp_PCC_7716.txt); do cp $word ../../NO_PALINDROMES/GCAATAGC/Orthologues_GCAATAGC_PCC_7716; done ## copio los ortólogos de la lista a la carpeta
```
## Filtrado de paralogos
voy a crear dos carpetas una que contenga archivos con paralogos y otra que solo tenga ortólogos
```{bash, eval = FALSE}
cd ../../NO_PALINDROMES/GCAATAGC/
...
```
```{python, eval = FALSE}
python3 ../../FiltradoParalogos.py Orthologues_GCAATAGC_PCC_7716/
```

Esto me dio como resultado 86 archivos sin paralogos y 25 archivos con paralogos los cuals omitiré por ahora.

Ahora edito el archivo fasta para que el encabezado incluya unicamente la especie
```{bash, eval = FALSE}
# /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/NO_PALINDROMES/GCAATAGC/Only_ORTHOLOGUES
cd Only_ORTHOLOGUES/
...
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/GCAATAGC/Only_ORTHOLOGUES/
for f in *.awk1; do mafft $f >$f.mafft;done
```

## Cambio de fasta a PHYLIP
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/GCAATAGC/
cd ..
...
```{python, eval = FALSE}
python3 ../../AlignmentPalindromeCoords.py Only_ORTHOLOGUES/ GCAATAGC PCC_7716
```
...
###
FILE	PAL	START	END
5539_NAD-P--FAD-dependent...fna.awk1.mafft.phy	gcaatagc	1227	1234
6948_ABC_transporter_subs...fna.awk1.mafft.phy	gcaatagc	955	962
8357_MlaD_family_protein.fna.awk1.mafft.phy	gcaatagc	556	563
8198_serine-threonine_pro...fna.awk1.mafft.phy	gcaatagc	104	111
7656_YebC-PmpR_family_DNA...fna.awk1.mafft.phy	gcaatagc	745	752
9481_cob-I-yrinic_acid_a-...fna.awk1.mafft.phy	gcaatagc	748	755
8252_tetratricopeptide_re...fna.awk1.mafft.phy	gcaatagc	2040	2047
8252_tetratricopeptide_re...fna.awk1.mafft.phy	gcaatagc	2223	2230
8252_tetratricopeptide_re...fna.awk1.mafft.phy	gcaatagc	2745	2752
9713_glycosyltransferase_...fna.awk1.mafft.phy	gcaatagc	661	668
... etc
###
```
...
En el archivo **Orthologues_Palindrome_sites.txt** hay 315 sitios en total.
Cargamos el primer sitio el cual corresponde a:

```{}
###
FILE	PAL	START	END
9798_DUF3110_domain-conta...fna.awk1.mafft.phy	cggcgccg	406	413
...
Filtro conteos bajos (**<=1**) y loops.
...
# **NIES-4071--10** weight>=1 ONLY LOOPS
```{r}
ggtree(Tree2) +
  geom_tiplab(color='firebrick', offset = .10, hjust=3.0)+
  geom_label(aes(label = node),show.legend = TRUE)
...
# **NIES-4105--10** weight>=1 ONLY LOOPS
```{r}
ggtree(Tree2) +
  geom_tiplab(color='firebrick', offset = .10, hjust=3.0)+
  geom_label(aes(label = node),show.legend = TRUE)
