---
title: "Reconstrucción Ancestral de Secuencias - CGGCGCCG - NIES-4071"
author: "Eduardo PM"
date: "2023-03-30"
...
#setwd("/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/CGGCGCCG")
knitr::opts_knit$set(root.dir = "/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/CGGCGCCG")
```
...
```{r}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/CGGCGCCG
#setwd("/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/CGGCGCCG")
Counts <- read.table("../../Markov_count_CGGCGCCG_2023-5-7_18hrs43mins_Octanuc_.txt", sep = "\t", header = TRUE)
...
Aqui podemos ver que  **NIES-4071** y **NIES-4105** son las que tienen mayores sitios (**403**) seguido de **Calothrix_sp_PCC_7716** con 366.


```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/CGGCGCCG
awk '{if ($5>=1 && $2=="Calothrix_sp_NIES-4071"){print $1}}' ../../Markov_count_CGGCGCCG_2023-5-7_18hrs43mins_Octanuc_.txt >Ortologos_CGGCGCCG_Calothrix_sp_NIES-4071.txt
```

**Ortologos_CGGCGCCG_Calothrix_sp_NIES-4071.txt** contiene 371 ortólogos los cuales estan presentes en las 6 especies y tienen por lo menos una ocurrencia del palindromo **CGGCGCCG**
```{bash, eval = FALSE}
###
5352_secA.fna
5354_sat.fna
...
5357_hypothetical_protein.fna
5385_lptB.fna
5387_CPBP_family_glutamic...fna
5398_glycosyltransferase.fna
5413_heme_oxygenase_-bili...fna
5426_LL-diaminopimelate_a...fna
...
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/CGGCGCCG
mkdir Orthologues_CGGCGCCG_NIES-4071  ## Creo Carpeta
cd ../../gbff_homologues/CalothrixspNIES-3974NIES-3974_f0_0taxa_algOMCL_e0_ ## Entro a la carpeta de homólogos
for word in $(cat ../../PALINDROMES/CGGCGCCG/Ortologos_CGGCGCCG_Calothrix_sp_NIES-4071.txt); do cp $word ../../PALINDROMES/CGGCGCCG/Orthologues_CGGCGCCG_NIES-4071; done ## copio los ortólogos de la lista a la carpeta
```
## Filtrado de paralogos
voy a crear dos carpetas una que contenga archivos con paralogos y otra que solo tenga ortólogos
```{bash, eval = FALSE}
cd ../../PALINDROMES/CGGCGCCG/
...
```
```{python, eval = FALSE}
python3 ../../FiltradoParalogos.py Orthologues_CGGCGCCG_NIES-4071/
```

Esto me dio como resultado 295 archivos sin paralogos y56 archivos con paralogos los cuals omitiré por ahora.

Ahora edito el archivo fasta para que el encabezado incluya unicamente la especie
```{bash, eval = FALSE}
# /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/CGGCGCCG/Only_ORTHOLOGUES
cd Only_ORTHOLOGUES/
...
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/CGGCGCCG/Only_ORTHOLOGUES/
for f in *.awk1; do mafft $f >$f.mafft;done
```

## Cambio de fasta a PHYLIP
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/CGGCGCCG/
cd ..
...
```{python, eval = FALSE}
python3 ../../AlignmentPalindromeCoords.py Only_ORTHOLOGUES/ CGGCGCCG NIES-4071
```
...
5385_lptB.fna.awk1.mafft.phy	cggcgccg	424	431
5720_ABC_transporter_perm...fna.awk1.mafft.phy	cggcgccg	582	589
9524_mtnA.fna.awk1.mafft.phy	cggcgccg	824	831
7123_IctB_family_putative...fna.awk1.mafft.phy	cggcgccg	528	535
9977_metG.fna.awk1.mafft.phy	cggcgccg	305	312
9977_metG.fna.awk1.mafft.phy	cggcgccg	975	982
5539_NAD-P--FAD-dependent...fna.awk1.mafft.phy	cggcgccg	35	42
9879_cysK.fna.awk1.mafft.phy	cggcgccg	810	817
... etc
###
```
