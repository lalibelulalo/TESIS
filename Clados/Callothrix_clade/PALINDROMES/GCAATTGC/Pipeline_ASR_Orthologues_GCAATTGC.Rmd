---
title: "Reconstrucción Ancestral de Secuencias - GCAATTGC - NIES-4071"
author: "Eduardo PM"
date: "2023-03-30"
...
#setwd("/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/GCAATTGC")
knitr::opts_knit$set(root.dir = "/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/GCAATTGC")
```
...
```{r}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/GCAATTGC
#setwd("/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/GCAATTGC")
Counts <- read.table("../../Markov_count_GCAATTGC_2023-5-8_13hrs20mins_Octanuc_.txt", sep = "\t", header = TRUE)
...
Aqui podemos ver que  **NIES-4071** y **Calothrix_sp_NIES-4105** son los que tienen mas sitios (**116**) seguido de **PCC_7716** con **243**.


```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/GCAATTGC
awk '{if ($5>=1 && $2=="Calothrix_sp_PCC_7716"){print $1}}' ../../Markov_count_GCAATTGC_2023-5-8_13hrs20mins_Octanuc_.txt >Ortologos_GCAATTGC_Calothrix_sp_NIES-4071.txt
```

**Ortologos_GCAATTGC_Calothrix_sp_NIES-4071.txt** contiene 220 ortólogos los cuales estan presentes en las 6 especies y tienen por lo menos una ocurrencia del palindromo **GCAATTGC**
```{bash, eval = FALSE}
###
5342_Ppx-GppA_family_phos...fna
5357_hypothetical_protein.fna
5376_50S_ribosome-binding...fna
5404_ftsH3.fna
5404_ftsH3.fna
5421_hypothetical_protein.fna
...
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/GCAATTGC
mkdir Orthologues_GCAATTGC_NIES-4071  ## Creo Carpeta
cd ../../gbff_homologues/CalothrixspNIES-3974NIES-3974_f0_0taxa_algOMCL_e0_ ## Entro a la carpeta de homólogos
for word in $(cat ../../PALINDROMES/GCAATTGC/Ortologos_GCAATTGC_Calothrix_sp_NIES-4071.txt); do cp $word ../../PALINDROMES/GCAATTGC/Orthologues_GCAATTGC_NIES-4071; done ## copio los ortólogos de la lista a la carpeta
```
## Filtrado de paralogos
voy a crear dos carpetas una que contenga archivos con paralogos y otra que solo tenga ortólogos
```{bash, eval = FALSE}
cd ../../PALINDROMES/GCAATTGC/
...
```
```{python, eval = FALSE}
python3 ../../FiltradoParalogos.py Orthologues_GCAATTGC_NIES-4071/
```

Esto me dio como resultado 146 archivos sin paralogos y 50 archivos con paralogos los cuals omitiré por ahora.

Ahora edito el archivo fasta para que el encabezado incluya unicamente la especie
```{bash, eval = FALSE}
# /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/GCAATTGC/Only_ORTHOLOGUES
cd Only_ORTHOLOGUES/
...
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/GCAATTGC/Only_ORTHOLOGUES/
for f in *.awk1; do mafft $f >$f.mafft;done
```

## Cambio de fasta a PHYLIP
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/GCAATTGC/
cd ..
...
```{python, eval = FALSE}
python3 ../../AlignmentPalindromeCoords.py Only_ORTHOLOGUES/ GCAATTGC NIES-4071
```
...
###
FILE	PAL	START	END
9879_cysK.fna.awk1.mafft.phy	gcaattgc	604	611
6812_ATP-binding_cassette...fna.awk1.mafft.phy	gcaattgc	501	508
6678_aroH.fna.awk1.mafft.phy	gcaattgc	214	221
5534_hypothetical_protein.fna.awk1.mafft.phy	gcaattgc	52	59
9300_prfC.fna.awk1.mafft.phy	gcaattgc	1585	1592
5986_isoprenyl_transferas...fna.awk1.mafft.phy	gcaattgc	481	488
8170_HlyD_family_efflux_t...fna.awk1.mafft.phy	gcaattgc	796	803
6948_ABC_transporter_subs...fna.awk1.mafft.phy	gcaattgc	871	878
8860_trypsin-like_peptida...fna.awk1.mafft.phy	gcaattgc	625	632
... etc
###
```
...
Filtro conteos bajos (**<=5**) y loops.
