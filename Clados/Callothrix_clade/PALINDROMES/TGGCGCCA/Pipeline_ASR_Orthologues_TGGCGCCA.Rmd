---
title: "Reconstrucción Ancestral de Secuencias - ORTÓLOGOS - TGGCGCCA - PCC_7716"
author: "Eduardo PM"
date: "2023-03-30"
...
#setwd("/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/TGGCGCCA/")
knitr::opts_knit$set(root.dir = "/home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/TGGCGCCA/")
```
...
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/
python3 CountPalsInOrthologues.py ../gbff_homologues/CalothrixspNIES-3974NIES-3974GCF002368395_f0_0taxa_algOMCL_e0_ pals.list orthologues.list 3
```
...
```{r}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/TGGCGCCA/
...
Aqui podemos ver que efectivamente **Calothrix_sp_PCC_7716** es la que tiene mayores sitios (**795**) seguido de **Calothrix_sp_NIES-4105** con 671.

Solo quiero aquellos donde **Calothrix_sp_PCC_7716** tenga un conteo mayor o igual a 1 ya que en ese sitio es donde voy a reconstruir la secuencia ancestral
```{bash, eval = FALSE}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/TGGCGCCA/
awk '{if ($5!=0 && $2=="Calothrix_sp_PCC_7716"){print $1}}' ../../Markov_count_TGGCGCCA_2023-5-7_18hrs43mins_Octanuc_.txt >Ortologos_TGGCGCCA_Calothrix_sp_PCC_7716.txt
```

**Ortologos_TGGCGCCA_Calothrix_sp_PCC_7716.txt** contiene 667 ortólogos los cuales estan presentes en las 6 especies y tienen por lo menos una ocurrencia del palindromo **TGGCGCCA**
...
5360_DUF11_domain-contain...fna
5362_trpA.fna
5367_larC.fna
5373_DUF751_family_protei...fna
5377_trpE.fna
...
for word in $(cat ../../PALINDROMES/TGGCGCCA/Ortologos_TGGCGCCA_Calothrix_sp_PCC_7716.txt); do cp $word ../../PALINDROMES/TGGCGCCA/Orthologues_TGGCGCCA_PCC_7716; done ## copio los ortólogos de la lista a la carpeta
```
## Filtrado de paralogos
voy a crear dos carpetas una que contenga archivos con paralogos y otra que solo tenga ortólogos
```{bash, eval = FALSE}
cd ../../PALINDROMES/TGGCGCCA/
...
```
Esto me dio como resultado 522 archivos sin paralogos y 97 archivos con paralogos los cuals omitiré por ahora.

Ahora edito el archivo fasta para que el encabezado incluya unicamente la especie
```{bash, eval = FALSE}
# /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/PALINDROMES/TGGCGCCA/Only_ORTHOLOGUES
cd Only_ORTHOLOGUES/
...
###
FILE	PAL	START	END
6349_serine_hydrolase.fna.awk1.mafft.phy	tggcgcca	513	520
7482_alpha-beta_hydrolase.fna.awk1.mafft.phy	tggcgcca	419	426
5720_ABC_transporter_perm...fna.awk1.mafft.phy	tggcgcca	957	964
8642_methyl-accepting_che...fna.awk1.mafft.phy	tggcgcca	1997	2004
8642_methyl-accepting_che...fna.awk1.mafft.phy	tggcgcca	2130	2137
7426_CoA-binding_protein.fna.awk1.mafft.phy	tggcgcca	730	737
6951_hypothetical_protein.fna.awk1.mafft.phy	tggcgcca	544	551
8371_NB-ARC_domain-contai...fna.awk1.mafft.phy	tggcgcca	692	699
7735_gshA.fna.awk1.mafft.phy	tggcgcca	474	481
... etc
###
```
...
```{r}
## /home/lalibelulalo/PIPELINES_2023/ASR_ORTHOLOGUES/Callothrix_clade/TGGCGCCA
...
Creo una matriz que tendra las transiciones y una columna extra con la direccion de la transición
```{r}
{LINKS <- matrix(0,
             nrow = 0,
             ncol = 3)
...
```
Cargo el **modelo de evolucion**, el metodo de reconstrucción (**bayesiano**), la **filogenia** (previamente calculada con **orthofinder**) la cual enraizo en **PCC_7716**.
```{r}
EvolModel = "F81"
Method = "bayes"
...
En el archivo **Orthologues_Palindrome_sites.txt** hay 608 sitios en total.
...
```
Al final de este paso tendre dos matrices **DF**, **LINKS** y **DELECIONES**
...
DF
```
**LINKS** las transiciones con sus respectivas direcciones.
```{r}
...
Finalmente repito esto para los **607** sitios palindromicos restantes:
...
En total son 6080 transiciones ya que son 10 transiciones por sitio y tenemos 608 sitios.
```{r}
length(DF$from)
length(LINKS$from)
length(DELECIONES[,1])
...
En total tengo 6080 transiciones:
```{r}
library(data.table)
